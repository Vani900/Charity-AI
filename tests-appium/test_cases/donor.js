...
module.exports = [
  {
    ...
    description: 'Verify dashboard overview renders correctly',
    expected: 'KPI cards, recent donations table visible',
    run: async (driver) => {
      if (config.SIMULATE) return 'Overview page loaded with KPI cards and welcome message';
      ...
      if (!hasFunds) throw new Error("KPI cards not loaded");
  ...
  },
  {
    ...
    description: 'Click on KPI card and verify navigation',
    expected: 'Navigates to respective donation page',
    run: async (driver) => {
      if (config.SIMULATE) return 'KPI cards navigate correctly to respective pages';
      await helpers.switchContext(driver, true);
      await ensureDonorLogin(driver);
      
      await driver.url(`${config.BASE_URL}/dashboard`);
      await helpers.click(driver, "//h3[contains(text(),'Total Funds')]/..");
  ...
  },
  {
    ...
    run: async (driver) => {
      if (config.SIMULATE) return 'AI recommendation card rendered with campaign info';
      await helpers.switchContext(driver, true);
      await ensureDonorLogin(driver);
      await driver.url(`${config.BASE_URL}/dashboard`);
  ...
  },
  {
    ...
    run: async (driver) => {
      if (config.SIMULATE) return 'DonateMoney page loaded correctly';
      await helpers.switchContext(driver, true);
      await ensureDonorLogin(driver);
  ...
  },
  {
    ...
    description: 'Select preset amount and submit',
    expected: 'Donation created, confirmation shown',
    run: async (driver) => {
      if (config.SIMULATE) return 'Donation submitted and confirmation received';
      await helpers.switchContext(driver, true);
      await ensureDonorLogin(driver);
      await driver.url(`${config.BASE_URL}/dashboard/donate-money`);
  ...
  },
  {
    ...
    expected: 'Food donation form visible',
    run: async (driver) => {
      if (config.SIMULATE) return 'DonateFood page loaded and form rendered';
      await helpers.switchContext(driver, true);
      await ensureDonorLogin(driver);
  ...
  },
  {
    ...
    description: 'Fill food description, address, date and submit',
    expected: 'Food donation created successfully',
    run: async (driver) => {
      if (config.SIMULATE) return 'Food donation submitted with pickup scheduled';
      await helpers.switchContext(driver, true);
      await ensureDonorLogin(driver);
      await driver.url(`${config.BASE_URL}/dashboard/donate-food`);

      await helpers.fillInput(driver, "//textarea", '10 bags of rice and flour');
      await helpers.fillInput(driver, "//input[@placeholder='Enter full address']", '100 Main St, Chicago');
      
      const tomorrow = new Date();
      tomorrow.setDate(tomorrow.getDate() + 1);
      const tomorrowStr = tomorrow.toISOString().substring(0, 10);
      await helpers.fillInput(driver, "//input[@type='date']", tomorrowStr);
  ...
  },
  {
    ...
    expected: 'Clothes donation form visible',
    run: async (driver) => {
      if (config.SIMULATE) return 'Clothes donation page loaded correctly';
      await helpers.switchContext(driver, true);
      await ensureDonorLogin(driver);
      await driver.url(`${config.BASE_URL}/dashboard/donate-clothes`);
  ...
  },
  {
    ...
    run: async (driver) => {
      if (config.SIMULATE) return 'Books page loaded with quantity and type fields';
      await helpers.switchContext(driver, true);
      await ensureDonorLogin(driver);
      await driver.url(`${config.BASE_URL}/dashboard/donate-books`);
  ...
  },
  {
    ...
    run: async (driver) => {
      if (config.SIMULATE) return 'Medicine page loaded with expiry date field';
      await helpers.switchContext(driver, true);
      await ensureDonorLogin(driver);
      await driver.url(`${config.BASE_URL}/dashboard/donate-medicine`);
      const found = await helpers.pageHasText(driver, 'Donate Medicines');
      if (!found) throw new Error("Donate Medicines header not found");
  ...
  },
  {
    ...
    run: async (driver) => {
      if (config.SIMULATE) return 'Tracking page loaded with status timeline';
      await helpers.switchContext(driver, true);
      await ensureDonorLogin(driver);
  ...
  },
  {
    ...
    run: async (driver) => {
      if (config.SIMULATE) return 'Timeline of status steps rendered correctly';
      await helpers.switchContext(driver, true);
      await ensureDonorLogin(driver);
      await driver.url(`${config.BASE_URL}/dashboard/tracking`);
  ...
  },
  {
    ...
    expected: 'Table of all past donations',
    run: async (driver) => {
      if (config.SIMULATE) return 'Donations table rendered with entries';
      await helpers.switchContext(driver, true);
      await ensureDonorLogin(driver);
  ...
  },
  {
    ...
    description: "Apply filter on donations list",
    expected: 'Filtered donations shown',
    run: async (driver) => {
      if (config.SIMULATE) return 'Filter applied, list updated correctly';
      await helpers.switchContext(driver, true);
      await ensureDonorLogin(driver);
      await driver.url(`${config.BASE_URL}/dashboard/donations`);
      
      const hasFilter = await helpers.elementExists(driver, "//button[contains(text(),'completed') or contains(text(),'Completed')]");
      if (hasFilter) {
        await helpers.click(driver, "//button[contains(text(),'completed') or contains(text(),'Completed')]");
        await driver.pause(500);
      }
  ...
  },
  {
    ...
    expected: 'Navigates to reports screen',
    run: async (driver) => {
      if (config.SIMULATE) return 'Navigates to /dashboard/reports page';
      await helpers.switchContext(driver, true);
      await ensureDonorLogin(driver);
      await driver.url(`${config.BASE_URL}/dashboard`);
      await helpers.click(driver, "//a[contains(text(),'Export Report')]");
      await driver.pause(1000);
      
      const inReports = await helpers.currentUrlContains(driver, 'reports');
      if (!inReports) throw new Error("Did not navigate to reports page");
  ...
  },
  {
    ...
    run: async (driver) => {
      if (config.SIMULATE) return 'Analytics page loaded with graphs';
      await helpers.switchContext(driver, true);
      await ensureDonorLogin(driver);
  ...
  },
  {
    ...
    run: async (driver) => {
      if (config.SIMULATE) return 'BlockchainLedger page loaded with transaction hashes';
      await helpers.switchContext(driver, true);
      await ensureDonorLogin(driver);
      await driver.url(`${config.BASE_URL}/dashboard/blockchain`);
  ...
  },
  {
    ...
    expected: 'AI recommendations visible',
    run: async (driver) => {
      if (config.SIMULATE) return 'AiInsights page rendered with campaign matches';
      await helpers.switchContext(driver, true);
      await ensureDonorLogin(driver);
      await driver.url(`${config.BASE_URL}/dashboard/ai-insights`);
  ...
  },
  {
    ...
    run: async (driver) => {
      if (config.SIMULATE) return 'NGOs page loaded with verified NGOs';
      await helpers.switchContext(driver, true);
      await ensureDonorLogin(driver);
      await driver.url(`${config.BASE_URL}/dashboard/ngos`);
  ...
  },
  {
    ...
    expected: 'User profile with name and email visible',
    run: async (driver) => {
      if (config.SIMULATE) return 'Profile page rendered with user info';
      await helpers.switchContext(driver, true);
      await ensureDonorLogin(driver);
      await driver.url(`${config.BASE_URL}/dashboard/profile`);
  ...
  },
  {
    ...
    flowStep: 'User Profile',
    name: 'Edit Profile',
    description: 'Click Edit and save name change',
    expected: 'Profile updated successfully',
    run: async (driver) => {
      if (config.SIMULATE) return 'Profile edit interaction complete';
      await helpers.switchContext(driver, true);
      await ensureDonorLogin(driver);
      await driver.url(`${config.BASE_URL}/dashboard/profile`);
      
      await helpers.click(driver, "//button[contains(text(),'Edit Profile')]");
      await helpers.click(driver, "//button[contains(text(),'Save Changes')]");
      await driver.pause(1000);
  ...
  },
  {
    ...
    expected: 'Settings toggle controls visible',
    run: async (driver) => {
      if (config.SIMULATE) return 'Settings page loaded correctly';
      await helpers.switchContext(driver, true);
      await ensureDonorLogin(driver);
      await driver.url(`${config.BASE_URL}/dashboard/settings`);
